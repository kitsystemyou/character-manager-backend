...
CREATE TABLE `users` (
    ...
    `username` char(50) NOT NULL COMMENT 'ユーザー名',
...
CREATE TABLE `coc_status_parameters` (
    `character_id` int NOT NULL AUTO_INCREMENT COMMENT 'キャラクターID',
    ...
    `remain_job_point` int  NULL COMMENT '職業ポイント残量',
    `max_concern_point` int  NULL COMMENT '最大興味ポイント',
    `remain_concern_point` int  NULL COMMENT '興味ポイント残量',
    FOREIGN KEY (`character_id`) REFERENCES characters (`id`)
) ENGINE = InnoDB  CHARSET = utf8mb4;
