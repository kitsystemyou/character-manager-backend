...
CREATE TABLE
    `characters` (
        ...
        `player_name` char(20) NULL COMMENT 'PL名',
        `game_system` char(20) NULL COMMENT 'ゲームシステム',
        `prof_img_path` char(50) NULL COMMENT 'プロフ画像パス',
        `tags` char(255) NULL COMMENT 'タグ(カンマ区切)',
        `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '作成日時',
        `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新日時',
        `delete_time` DATETIME NULL COMMENT '削除日時',
        PRIMARY KEY (`id`)
